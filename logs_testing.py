...
#Problem 3 (corrupted config files)
...
output="Corrupted config file found : please delete file `%GROUP1%` (type %GROUP2%)."
print(search_content(re, content, output), end="")

#Problem 4 (red herring errors)
re="Cowardly refusing to send event"
output="Only errors before the first `Cowardly refusing to send event` are the cause!"
print(search_content(re, content, output), end="")
...
re=r"module (mixinextras\.neoforge|MixinExtras) exports package [a-zA-Z0-9_.-]* to (mixinextras\.neoforge|MixinExtras)"
...
#Problem 9.2 (MixinExtras wrong modloader)
re=r"Module (?:mixinextras.neoforge|MixinExtras) contains package [a-zA-Z0-9_.-]+, module (?:mixinextras.neoforge|MixinExtras) exports package [a-zA-Z0-9_.-]+ to (?:mixinextras.neoforge|MixinExtras)"
...
#Problem 13 (Unsupported mods in profile)
...
The mod name is contained in the following path: `%GROUP1%`. It has been compiled to version `%GROUP3%` but this version of the Java Runtime only recognizes class file versions up to `%GROUP4%`."""
print(search_content(re, content, output), end="")
